...
<div class="price">
    ...
    <?= $model->isNegotiable() ? '<span>' . t('app', 'Negotiable') . '</span>' : ''; ?></div>
<div class="image">
    <?= !empty($model->showPromotedLabel) ? '<div class="promoted" style="background-color: '. $model->package->promo_label_background_color .'"><span style="color:'. $model->package->promo_label_text_color .'">' . $model->package->promo_label_text . '</span></div>' : '' ?>
...
<div class="info">
    ...
    <a href="<?= url(['/listing/index/', 'slug' => $model->slug]); ?>" class="name">
        <?= html_encode($model->title); ?>
        <span><?= StringHelper::truncate(strip_tags(html_purify($model->description)), 80); ?></span>
    </a>
    <div class="location"><i class="fa fa-map-marker" aria-hidden="true"></i> <?= html_encode($model->getZoneCountryString()); ?>
    </div>
</div>