...
?>
<div id="listings-search-map" class="listing-on-map">
    ...
    <div class="map-wrapper">
        ...
        <!-- Content for notify Error -->
        <div class="map-error" style="text-align: center"></div>
        <!-- End Content-->

        <div class="map" id="map"
             data-msg-error="<?= t('app', 'Address not found, please insert another location.')?>"
             data-url="<?=url(['/site/get-map-location']);?>" style="height: 300px; width: 100%; background-color: #eeebe8; filter: blur(10px);
            ...
            function getCoord() {
                ...
                if(locationDetails && locationDatabase) {
                    jQuery.post(jQuery('#map').data('url'),{
                        ...
                        geocoder.geocode({'address': json.content.country + ', ' + json.content.zone + ', ' + json.content.city}, function (results, status) {
                            if (status === google.maps.GeocoderStatus.ZERO_RESULTS) {

                                //show notify error

                                notify.addWarning($('#map').data('msg-error'));
                                notify.show();
...
<section class="main-search">
    <?= $this->render('_searchpage-search-map', [
...
<div class="container">
    <div class="row">
        <div class="col-lg-8 col-lg-push-2 col-md-8 col-md-push-2 col-sm-12 hidden-xs">
            <?php app()->trigger('search.map.bottom', new \app\yii\base\Event()); ?>
        </div>
    </div>
</div>
<div class="container">
    <div class="row">
        <div class="mobile-advert mb10 col-xs-12 hidden-lg hidden-md hidden-sm">
            <?php app()->trigger('mobile.search.map.bottom', new \app\yii\base\Event()); ?>
        </div>
    </div>
</div>