...
class ListingImage extends \app\models\auto\ListingImage
{
    ...
    public function uploadImageByAjax($imagesGallery, $image_form_key, $adId, $sort)
    {
        ...
        foreach ($imagesGallery as $imageGallery) {
            ...
            $newGalleryImageName = preg_replace('/\s+/', '', $newGalleryImageName);
     ...
     * @return mixed|null
     */
    public function getAdsImageRestrictionsSize()
    {
        if ((options()->get('app.settings.common.imgMaxSize',0)) != 0) {
            $maxImageSize = options()->get('app.settings.common.imgMaxSize',0);
        } else {
            $maxImageSize = null;
        }
