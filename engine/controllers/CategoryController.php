...
class CategoryController extends Controller
{
    ...
    public function actionIndex($slug)
    {
        ...
        $this->setViewParams([
            ...
            'pageMetaDescription'       => isset($category->meta_description) ? $category->meta_description : 'Default',
            'pageMetaKeywords'          => isset($category->meta_keywords) ? $category->meta_keywords : 'Default'
        ]);
    ...
    public function actionMapView($slug)
    {
        ...
        $AdsProvider = $searchModel->categorySearch($params);
        $AdsProvider->query->andWhere(['category_id' => $categories])
            ->orderBy(['promo_expire_at' => SORT_DESC, 'created_at' => SORT_DESC]);
        $AdsProvider->sort = ['defaultOrder' => ['promo_expire_at' => SORT_DESC]];
        $AdsProvider->pagination = [
        ...
        $locationDetails = '';
        $locationDatabase = '';
        if (isset($params['ListingSearch']) && !empty($params['ListingSearch']['location'])) {
            $locationDatabase = $params['ListingSearch']['location'];
            if(strpos($params['ListingSearch']['location'], 'zo-') === 0){
                $zone = Zone::find()->with('country')->where(['zone_id' => substr($params['ListingSearch']['location'],3)])->one();
                if ($zone) {
                    $locationDetails = $zone->name . ', ' . $zone->country->name;
                }
            } else if(strpos($params['ListingSearch']['location'], 'co-') === 0){
                $country = Country::find()->where(['country_id' => substr($params['ListingSearch']['location'],3)])->one();
                if ($country) {
                    $locationDetails = $country->name;
                }
            } else if(strpos($params['ListingSearch']['location'], 'ci-') === 0){
                $location = Location::find()->where(['LIKE', 'city', substr($params['ListingSearch']['location'],3)])->one();
                if ($location) {
                    $locationDetails = $location->city . ', ' . $location->country->name;
                } else {
                    $locationDetails = html_encode(substr($params['ListingSearch']['location'],3));
                }
            }
        }
