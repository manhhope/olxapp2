...
class ListingController extends Controller
{
    ...
    public function actionIndex($slug)
    {
        ...
        $mainImage = $ad->mainImage->image_path;

        $this->setViewParams([
            'pageTitle'                 => $ad->title . ' - {siteName}',
            'pageMetaDescription'       => trim($ad->metaDescription),
            'pageMetaKeywords'          => '',
            'pageMetaImage'             => Url::base(true) . $mainImage
        ]);

        app()->on('app.header.beforeScripts', function () use ($mainImage, $ad) {
            echo '<meta property="og:image:width" content="' . ListingImage::IMG_WIDTH . '" />' . PHP_EOL;
            echo '<meta property="og:image:height" content="' . ListingImage::IMG_HEIGHT . '" />' . PHP_EOL;
        });
    ...
    public function actionPost()
    {
        ...
        $this->setViewParams([
            'pageTitle'                 => options()->get('app.settings.common.postTitle', 'Post Ad - {siteName}'),
            'pageMetaDescription'       => options()->get('app.settings.common.postDescription', 'Post an Ad listing now'),
            'pageMetaKeywords'          => options()->get('app.settings.common.postKeywords', 'Ads, Classified ads, sell, buy, trade, market')
        ]);
    ...
    public function actionUpdate($slug)
    {
        ...
        $this->setViewParams([
            'pageTitle'                 => t('app', 'Update Ad') . ' - {siteName}',
        ]);
    ...
    public function actionPackage($slug)
    {
        ...
        if (options()->get('app.settings.common.skipPackages', 0) == 1){
            $this->skipPackages($ad);
            return false;
        }
        ...
        $this->setViewParams([
            'pageTitle'                 => 'Package: ' .  $ad->title . ' - {siteName}',
        ]);
     ...
     * @return array|Response
     */
    public function actionGetSortedImages()
    {
        /* allow only ajax calls */
        if (!request()->isAjax) {
            return $this->redirect(['listing/post']);
        ...
        $imageFormKey   = request()->post('image_form_key');
        $action         = request()->post('action');
        $adId           = request()->post('ad_id');

        if (empty($imageFormKey) || empty($action)) {
            return ['result' => 'error', 'html' => t('app', 'Something went wrong...')];
        }

        // to know what to fetch images with form_key or images with listing_id
        $condition = [];
        if($action != 'update') {
            $condition = [
                'image_form_key' => $imageFormKey
            ];
        ...
        $images = ListingImage::find()->where($condition)->orderBy(['sort_order' => SORT_ASC])->all();

        $sortedImages = [];
        foreach ($images as $key=>$image) {
            $sortedImages[$key]['image_id'] = $image->image_id;
            $sortedImages[$key]['resized_image'] = $image->image;
            $sortedImages[$key]['original_image'] = $image->image_path;
        }
