...
class CommonHelper
{
    ...
    public static function verifyLicense(License $model = null)
    {
        ...
        $marketing = (!empty($model->marketing)) ? 'I want to receive marketing emails and newsletters about CodinBit products' : 'delete';
     ...
     * @return string
     */
    public static function getSymbolOfCurrency($currencyCode)
    {
        return Currency::find()->select(['symbol'])->where([
            'code' => $currencyCode,
     ...
     */
    public static function getPriceAsCurrency($currencyCode, $price)
    {
        if(!$currencyCode) return false;

        $symbol = static::getSymbolOfCurrency($currencyCode);
        if(!$symbol) return false;

        // docs for custom currency
        //https://github.com/yiisoft/yii2/issues/13629#issuecomment-314283147

        app()->formatter->numberFormatterSymbols = [
            \NumberFormatter::CURRENCY_SYMBOL => $symbol->symbol
        ];

        return app()->formatter->asCurrency($price, '', [
    ...
    public static function getEntryScriptUrl()
    {
        ...
        if (basename($_SERVER['SCRIPT_NAME']) === $scriptName) {
            ...
            $entryScript = $_SERVER['PHP_SELF'];
        } elseif (isset($_SERVER['ORIG_SCRIPT_NAME']) && basename($_SERVER['ORIG_SCRIPT_NAME']) === $scriptName) {
            $entryScript = $_SERVER['ORIG_SCRIPT_NAME'];
        } elseif (($pos = strpos($_SERVER['PHP_SELF'],'/'.$scriptName)) !== false) {
            $entryScript = substr($_SERVER['SCRIPT_NAME'],0,$pos).'/'.$scriptName;
        } elseif (isset($_SERVER['DOCUMENT_ROOT']) && strpos($_SERVER['SCRIPT_FILENAME'], $_SERVER['DOCUMENT_ROOT']) === 0) {
            $entryScript = str_replace('\\','/', str_replace($_SERVER['DOCUMENT_ROOT'], '', $_SERVER['SCRIPT_FILENAME']));
        } else {
