$(document).ready(function(){
    ...
    // change the width of the button container from list view
    if ($('.fa-envelope-o').length) {
        $('#table-action-customer').css('width','170px');
    } else {
        $('#table-action-customer').css('width','145px');
    }
    ...
    if ($('#admin-contact-form').length) {
        if ($('#latitude-address').val() == 0 && $('#longitude-address').val() == 0) {
            setTimeout(function() {
                initMap(1, 44.429459, 26.046170);
            }, 500);
        } else {
            setTimeout(function() {
                initMap(10, $('#latitude-address').val(), $('#longitude-address').val());
            }, 500);
        }
    }


    $(document).on('blur','#contact-country, #contact-zone, #contact-city, #contact-zip', function (e) {
        var zoom = 7;
        ...
        var interval = setInterval(getCoord, 1000);
        ...
        function getCoord() {
            ...
            notify.setOption('container', '.map-error');
            notify.remove();

            var geocoder = new google.maps.Geocoder();
            geocoder.geocode({'address': country + ', ' + zone + ', ' + city + ', ' + zip}, function (results, status) {

                if (status === google.maps.GeocoderStatus.ZERO_RESULTS) {

                    notify.addWarning($('#map-msg').data('msg-error'));
                    notify.show();
                ...
                if (status === google.maps.GeocoderStatus.OK) {
                    ...
                    initMap(zoom, results[0].geometry.location.lat(), results[0].geometry.location.lng());

                    var lat = results[0].geometry.location.lat().toFixed(7);
                    var lng = results[0].geometry.location.lng().toFixed(7);

                    $('#latitude-address').val(lat);
                    $('#longitude-address').val(lng);
    ...
    $('select#common-confirmationemail').on('change', function(){
        var $this = $(this);
        if ($this.val() == 1) {
            $('#expire-confirmation-key').show();
        } else {
            $('#expire-confirmation-key').hide();
        }
    });

    $('#listings-disablestore').on('change',function () {
        if ($('#listings-disablestore').val() === '1') {
            if(!confirm($('#listings-disablestore').data('message'))){
                return false;
            }
        }
    ...
    $('select#listings-skippackages').on('change', function(){
        var $this = $(this);
        if ($this.val() == 1) {
